    ...
    <div class="container p-4 mx-auto mt-1">
        <h1 class="mb-3 text-2xl lg:text-4xl text-bold dark:text-white"><span
                class="text-gray-500 hover:text-gray-950 dark:hover:text-white"><a
                    href="{{ route('detailkesalahan-siswa.index') }}">DETAIL KESALAHAN</a></span>->Update Detail Kesalahan
        </h1>
        @if (session('msg'))
        ...
        <div class="w-full p-4 overflow-x-auto bg-white rounded-lg shadow-md">
            <form action="{{ route('detailkesalahan-siswa.update', $data->id) }}" method="post"
                ...
                <div class="flex flex-col grid-cols-3 gap-4 mb-3 lg:grid">
                    ...
                    <div class="flex flex-col lg:col-span-2">
                        <input type="text" value="{{ $data->student->name }}" name="txtname" id="txtname"
                            autocomplete="off" autofocus class="w-full text-xs rounded lg:text-sm" maxlength="255"
                ...
                <div class="flex flex-col grid-cols-3 gap-4 mb-3 lg:grid">
                    ...
                    <input type="text" value="{{ $data->student->kelas }}" name="txtkelas" id="txtkelas"
                        autocomplete="off" autofocus class="text-xs rounded lg:text-sm lg:col-span-2" readonly>
                </div>
                <div class="flex flex-col grid-cols-3 gap-4 mb-3 lg:grid">
                    <label for="txtkesalahanId" class="text-xs lg:text-sm">Jenis Kesalahan</label>
                    <div class="flex flex-col lg:col-span-2">
                        <select name="txtkesalahanId" id="txtkesalahanId" class="text-xs rounded lg:text-sm lg:col-span-2"
                            required>
                            <option value="">Pilih</option>
                            @foreach ($kesalahan as $p)
                                <option value="{{ $p->id }}" {{ $data->kesalahan_id == $p->id ? 'selected' : '' }}>
                                    {{ $p->name }}</option>
                            @endforeach
                        </select>
                        @error('txtkesalahanId')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                </div>
                ...
                <div class="flex flex-col grid-cols-3 gap-4 mb-3 lg:grid">
                    ...
                    <div class="flex flex-col lg:col-span-2">
                        <textarea name="txtketerangan" id="txtketerangan" cols="30" rows="5"
                            class="text-xs rounded lg:text-sm lg:col-span-2" maxlength="255" required>{{ $data->keterangan }}</textarea>
                        @error('txtketerangan')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
